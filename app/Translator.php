...
class Translator implements ArrayAccess, JsonSerializable
{
    ...
    public function __construct(Model $model)
    {
        ...
        $this->model = $model;
        $this->locale = config('app.locale', 'zhtw');
        $attributes = [];
    ...
    public function save()
    {
        ...
        foreach ($attributes as $key => $attribute) {
            if ($attribute['exists']) {
                ...
                $translation = app(Translation::class)->where('table_name', $this->model->getTable())
    ...
    public function deleteTranslation($key)
    {
        ...
        app(Translation::class)->where('table_name', $this->model->getTable())
            ->where('column_name', $key)
            ->where('foreign_key', $this->model->getKey())
            ->where('locale', $locale)
            ->delete();
    ...
    public function __call($method, array $arguments)
    {
        if (!$this->model->hasTranslatorMethod($method)) {
            throw new \Exception('Call to undefined method App\Translator::'.$method.'()');
        }
